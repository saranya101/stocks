...
def calculate_signal(ticker, timeframe="5m"):
    stock = yf.download(
        ticker,
        period="5d",
        interval=timeframe,
    ...
    latest_fast = safe_float(sma_fast.iloc[-1])
    latest_slow = safe_float(sma_slow.iloc[-1])
    latest_rsi = safe_float(rsi.iloc[-1])

    recent_volume = safe_float(volume.tail(10).mean())
